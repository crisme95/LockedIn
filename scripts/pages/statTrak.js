 ...
 */
export const init = Object.freeze(function () {
    displaySessionStats();
});
 ...
 *
 * If no statistics are available (totalTime = 0),
 * displays a placeholder message.
 */
export const displaySessionStats = Object.freeze(async function () {
    ...
    const productivePercentage = Math.round(
        (stats.productiveTime / totalTime) * 100
    );
    const distractingPercentage = Math.round(
        (stats.distractingTime / totalTime) * 100
    );

    // Convert session duration from milliseconds to minutes and seconds
    const sessionDurationInSeconds = Math.round(
        (stats.sessionEnd - stats.sessionStart) / 1000
    );
    ...
    container.innerHTML = `
            ...
            <p>
                Session Duration: ${sessionMinutes} minutes and 
            ...
            <p>
                ...
                ${Math.round(stats.productiveTime / 1000 / 60)} minutes
            </p>
            <p>
                Time spent on distracting sites: 
                ${Math.round(stats.distractingTime / 1000 / 60)} minutes
            </p>
